...
function make_exogenous_stage_builder(inst::InstanceData, t::Int)
...
stages_std = Stage[
    Stage(1, nI, make_exogenous_stage_builder(inst, 1),
          ()  -> expected_d1, unit_weight,
          (t, ctx) -> (Any[saa[1][n, :] for n in 1:N_SAA], uniform_p),
          (t, ctx, ω) -> nothing, x -> nothing),
    Stage(2, nI, make_exogenous_stage_builder(inst, 2),
          ()  -> saa[1][rand(rng_std, 1:N_SAA), :], unit_weight,
          (t, ctx) -> (Any[saa[1][n, :] for n in 1:N_SAA], uniform_p),
          (t, ctx, ω) -> nothing, x -> nothing),
    Stage(3, nI, make_exogenous_stage_builder(inst, 3),
          ()  -> saa[1][rand(rng_std, 1:N_SAA), :], unit_weight,
          (t, ctx) -> (Any[saa[1][n, :] for n in 1:N_SAA], uniform_p),
          (t, ctx, ω) -> nothing, x -> nothing),
    Stage(4, nI, make_exogenous_stage_builder(inst, 4),
          ()  -> saa[1][rand(rng_std, 1:N_SAA), :], unit_weight,
          (t, ctx) -> (Any[saa[1][n, :] for n in 1:N_SAA], uniform_p),
          (t, ctx, ω) -> nothing, x -> nothing),
]
...
t_std_start   = time()
ω1_std        = expected_d1   # fixed deterministic demand for stage-1 LB
...
function simulate_policy(m, is_ddu::Bool; n_paths::Int = N_SIM, seed::Int = 1111)
    ...
    for s in 1:n_paths
        ...
        for t in 1:inst.T
            ...
            ξ      = Float64[rand(rng, _customer_dist(inst, j, za))
                              for j in 1:inst.num_customers]
...
out_dir = joinpath(@__DIR__, "..")
CSV.write(joinpath(out_dir, "comparison_summary.csv"), summary_df)
...
CSV.write(joinpath(out_dir, "convergence_data.csv"), conv_df)
...
savefig(fig, joinpath(out_dir, "convergence_plot.png"))

println("\nFiles written to: $out_dir")
println("  comparison_summary.csv")
println("  convergence_data.csv")
println("  convergence_plot.png")
