...
const N_SAA     = length(ARGS) >= 2 ? parse(Int, ARGS[2]) : 50
...
# Pre-generate scenario draws once, shared across both models for each dist.
# :saa   → one row index into saa[d] per (path, stage)
# :exact → one UInt32 seed per (path, stage) used to create a local RNG,
#           so sampling is independent of how many draws previous stages consumed.
...
function simulate_policy(m, is_ddu::Bool, dist::Symbol; n_paths::Int = N_SIM)
    ...
    for s in 1:n_paths
        ...
        for t in 1:inst.T
            ...
            else  # :exact — local RNG isolates this draw from all other (s,t) pairs
...
profits_ddu_true, x_hist_ddu_true = simulate_policy(m_ddu, true,  :exact)
println("Simulating DDU-SDDiP — SAA distribution   ($N_SIM paths) ...")
profits_ddu_saa,  x_hist_ddu_saa  = simulate_policy(m_ddu, true,  :saa)
